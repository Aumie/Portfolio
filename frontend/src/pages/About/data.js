export const job = [
    {
		id: 'mJrecILWObon696Nf',
		order: 4,
		title: 'Software Engineer',
		dates: 'Jan 2024 - Current',
		duties: [
            "Gathered requirements with a System Analyst",
			'Mostly, worked with existing project. Maintained and implemented new features.',
			'Implemented both front-end and back-end',
			"Supported the customers, when there are issues on the products",
		],
		company: 'Motif',
	},
	{
		id: 'recIL6mJNfWObon69',
		order: 3,
		title: 'Unreal Engine Developer',
		dates: 'Sep 2023 - Oct 2023',
		duties: [
			'Implemented game mechanics',
			'Implemented a simple GOAP AI Framework to use in an Unreal Engine project',
			"Implemented a VR plugin to be used in V-mall project. Features: Changing product's color automaticcally while picking from a carousel with simple replaceable UI",
		],
		company: 'AloneClone',
	},
	{
		id: 'recIL6mJNfWObonls',
		order: 2,
		title: 'Jr. Solution Architect',
		dates: 'July 2021 - Mar 2022',
		duties: [
			'Provided quotations and service mapping architecture diagram to the customers after discussing with Sr. Solution Architects and Business Developments.',
			'Supported the customers by providing documentation or instructions through online meeting, when there are technical issues on products.',
			'Delivered solutions to the customer with Cloud Engineers.',
			'Worked off-site or online with a Sr. Solution Architect on requested Hands-On Labs from the customers.',
		],
		company: 'Huawei',
	},
	{
		id: 'rec61x18GVY99hQq5',
		order: 1,
		title: 'Software Engineer (Internship)',
		dates: 'Jun 2020 - Sep 2020',
		duties: [
			'Successfully developed a security system which IP cameras are used to detect and calculate visitors, entering or leaving a building, by using deep learning.',
			'Independently experimented with test frameworks i.e. MStest, Nunit, unittest, googletest.',
			'Experienced in creating CI/CD pipeline with Jenkins and Docker.',
		],
		company: 'NDR Solution',
	},
]
